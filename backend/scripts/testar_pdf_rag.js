...
import { extrairTextoPDF, estruturarDadosPDF } from '../src/services/pdf.service.js';
import { gerarResposta } from '../src/services/openai.service.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

// Caminho para o PDF de teste — ajuste conforme necessário
const PDF_TESTE = path.join(__dirname, '../src/database/pdfs/laudo_analise_solo_regenerativo.pdf');
...
async function rodarTeste() {
  ...
  for (const pergunta of PERGUNTAS_TESTE) {
    ...
    const resposta = await gerarResposta(pergunta, dadosPDF);
    ...
    resposta.split('\n').forEach((linha) => {
      ...
      const preview = linha.length > 50 ? linha.substring(0, 47) + '...' : linha;
  ...
  const respostaGenerica = await gerarResposta(perguntaGenerica, null);
